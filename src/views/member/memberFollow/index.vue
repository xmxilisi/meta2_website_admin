  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="关注人" prop="fromMemberId">
          <el-input v-model="form.fromMemberId" placeholder="请输入关注人" />
        </el-form-item>
        <el-form-item label="被关注人" prop="toMemberId">
          <el-input v-model="form.toMemberId" placeholder="请输入被关注人" />
        </el-form-item>
        <el-form-item label="是否互相关注：0否 1是" prop="isMutual">
          <el-input v-model="form.isMutual" placeholder="请输入是否互相关注：0否 1是" />
        </el-form-item>
      </el-form>
...
<script>
import { listMemberFollow, getMemberFollow, delMemberFollow, addMemberFollow, updateMemberFollow } from "@/api/member/memberFollow";
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        fromMemberId: [
          { required: true, message: "关注人不能为空", trigger: "blur" }
        ],
        toMemberId: [
          { required: true, message: "被关注人不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    handleUpdate (row) {
      ...
      getMemberFollow(id).then(response => {
        this.loading = false;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateMemberFollow(this.form).then(response => {
            ...
            addMemberFollow(this.form).then(response => {
