      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            type="text"
            icon="el-icon-edit"
            @click="handleComment(scope.row.postId)"
            v-hasPermi="['community:posts:post']"
          >查看评论回复</el-button>
          <el-button
            size="mini"
            type="text"
...
export default {
  ...
  methods: {
    ...
    /** 查看评论回复 */
    handleComment(id){
      this.$router.push({ path:'/threads/posts2', query: {thread_id: this.queryParams.threadId, post_id: id}});
    },
  }
};
</script>
