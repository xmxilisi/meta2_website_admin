    ...
    <el-row :gutter="20">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['community:newsChinese:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['community:newsChinese:remove']"
        >删除</el-button>
      </el-col>
         <el-button
           ...
           size="mini"
           @click="handleDraft"
           v-hasPermi="['community:newsChinese:draft']"
         >草稿箱</el-button>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['community:newsChinese:comment']"
          >查看评论</el-button>
          <el-button
            size="mini"
            type="text"
            ...
            v-hasPermi="['community:newsChinese:edit']"
          >修改</el-button>
          <el-button
            size="mini"
            type="text"
...
<script>
import {listNewsChinese, changeStatus} from "@/api/community/newsChinese";
import { treeselectChinese } from "@/api/community/newsType";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listNewsChinese(this.queryParams).then(response => {
    ...
    getTreeselect() {
      treeselectChinese().then(response => {
        ...
        const data = { id: 0, label: '全部分类', parentId: 0, weight: 0, children: [] };
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.push({ path: '/newsChinese/detail'})
    },
    /** 查看评论 */
    handleComment(nid){
      this.$router.push({ path:'/newsChinese/comment', query: {news_id: nid}})
    },
    /** 修改按钮操作 */
    handleUpdate(nid) {
      this.$router.push({ path:'/newsChinese/detail', query: {news_id: nid}})
    },
    /** 删除按钮操作 */
    handleDelete(row) {
