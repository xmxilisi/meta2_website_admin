<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="commentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="id" align="center" prop="commentId" v-if="true"/>
      <el-table-column label="发表用户" align="center" prop="memberName">
        <template slot-scope="scope">
          <span>{{ scope.row.memberName }}</span><span>{{ scope.row.nickname }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            type="text"
            icon="el-icon-edit"
            @click="handleComment(scope.row.commentId)"
            v-hasPermi="['community:comment:post']"
          >查看评论回复</el-button>
          <el-button
            size="mini"
            type="text"
...
export default {
  ...
  methods: {
    ...
    /** 查看评论回复 */
    handleComment(id){
      this.$router.push({ path:'/newsChinese/comment2', query: {news_id: this.queryParams.newsId, comment_id: id}});
    },
  }
};
</script>
