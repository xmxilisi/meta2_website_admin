...
<script>
import {addNews, getNews, updateNews} from "@/api/community/news";
import {listNewsType, treeselect} from "@/api/community/newsType";
...
export default {
  ...
  created() {
    ...
    if(this.$route.query.news_id){
      document.title = '修改资讯';
      getNews(this.$route.query.news_id).then(response => {
        //查询级联分类
